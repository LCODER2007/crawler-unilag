...
DETAILED_CACHE = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'data', 'unilag_staff_detailed.json')
...
FACULTY_SEEDS = [
    ...
    ("Faculty of Environmental Sciences", "https://environmentalsciences.unilag.edu.ng/staff/"),
...
NOISE = {'click','view','department','faculty','university','college','profile',
         'contact','email','phone','office','research','publications','more',
         'read','about','staff','list','home','menu','search','login','logout'}
...
class FacultyDirectorySpider(scrapy.Spider):
    ...
    def start_requests(self):
        headers = {'User-Agent': 'Mozilla/5.0 (compatible; URAAS/1.0; mailto:library@unilag.edu.ng)'}
    ...
    def parse_staff_page(self, response):
        ...
        for name in response.css('h2.elementor-cta__title::text, h3.elementor-heading-title::text, h4.elementor-heading-title::text').getall():
            if self._add(name, faculty, department):
                found += 1

        # Strategy 2: Bold text in content blocks
        for name in response.css('div.kc_text_block strong::text, div.entry-content strong::text, .wp-block-column strong::text').getall():
            if self._add(name, faculty, department):
                found += 1

        # Strategy 3: Table first column
        for name in response.css('table td:first-child::text, table td:nth-child(2)::text').getall():
            if self._add(name, faculty, department):
                found += 1

        # Strategy 4: List items
        for name in response.css('li strong::text, .staff-name::text, .member-name::text, .team-member-name::text').getall():
            if self._add(name, faculty, department):
                found += 1
        ...
        for link in response.css('a'):
            ...
            if any(kw in text.lower() for kw in ['department', 'dept', 'staff', 'academic']):
    ...
    def closed(self, reason):
        if not self.staff_records:
            self.logger.warning("No staff found — UNILAG site structure may have changed.")
            return
        ...
        self.logger.warning(f"Saved {len(unique_names)} staff names + {len(self.staff_records)} detailed records")
