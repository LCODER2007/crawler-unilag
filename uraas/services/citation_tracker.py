...
class CitationTracker:
            ...
            if oa_data:
                ...
                for cite in oa_data['citations']:
                    if cite['doi']:
                        ...
                        citing_item = session.query(Item).filter_by(doi=cite['doi']).first()
                        if citing_item:
                            ...
                            if not existing:
                                citation = Citation(
                                    citing_item_id=citing_item.id,
                                    cited_item_id=item_id,
                                    citation_date=datetime(cite['year'], 1, 1) if cite['year'] else None,
                                    source='openalex'
                                )
                                session.add(citation)
                
                session.commit()
                logger.info(f"Updated citations for item {item_id}: {citation_count} citations")
                return True
    ...
    def update_author_metrics(author_id: int) -> bool:
        ...
        try:
            ...
            papers = session.query(Item, CitationMetrics).join(
                Item.authors
            ).outerjoin(
            ...
            session.commit()
            logger.info(f"Updated metrics for author {author.name}: h-index={h_index}, citations={total_citations}")
            return True
...
def get_paper_citations(item_id: int) -> Dict:
    ...
    try:
        ...
        # Get citing papers
        citations = session.query(Citation, Item).join(
            Item, Citation.citing_item_id == Item.id
        ).filter(
            Citation.cited_item_id == item_id
        ).all()
...
def get_author_bibliometrics(author_id: int) -> Dict:
    ...
    try:
        ...
        return {
            ...
            'citations_per_paper': round(metrics.total_citations / metrics.total_papers, 1) if metrics.total_papers else 0
        }
    finally:
        session.close()
